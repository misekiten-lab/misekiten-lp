    ...
    <form onSubmit={handleSubmit} className="space-y-5" aria-label="無料店舗診断のお問い合わせフォーム">
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="block text-sm font-bold text-[#111827]">
          お名前 <span className="text-[#e85d4b]">必須</span>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="block text-sm font-bold text-[#111827]">
          メールアドレス <span className="text-[#e85d4b]">必須</span>
      ...
      <label className="block text-sm font-bold text-[#111827]">
        ご相談内容 <span className="text-[#e85d4b]">必須</span>
      ...
      <p className="text-xs leading-6 text-[#66716e]">送信いただいた情報は、お問い合わせへの回答以外には使用しません。</p>
      <button type="submit" disabled={state === "submitting"} className="flex min-h-14 w-full items-center justify-center gap-2 bg-[#f97360] px-6 py-4 text-base font-bold whitespace-nowrap text-white transition hover:bg-[#e85d4b] disabled:cursor-wait disabled:opacity-65">
        <PaperPlaneTilt size={21} weight="fill" aria-hidden="true" />
        {state === "submitting" ? "送信中です" : "無料診断を申し込む"}
      </button>
      {message ? (
        <p role="status" className={`flex items-start gap-2 border px-4 py-3 text-sm leading-6 ${state === "success" ? "border-[#b8dfd4] bg-[#edf8f4] text-[#0f6a5b]" : "border-[#f2bdb4] bg-[#fff4f1] text-[#9f3325]"}`}>
